...
with open("nf-core_pangenome authors - Sheet1.tsv", newline='') as f:
    ...
    for rank, last_name, first_name, affiliations_string, contributions_string, email in authors:
        ...
        latex_code += f"\\author{{\\fnm{{{first_name}}} \\sur{{{last_name}}}}}{email_}[{','.join(affiliation_indices)}{corresponding}]\n"
            
    for i in range(1, affiliation_index):
        latex_code += f"$^{i}${affiliation_dict_r[i]} \\\\ \n"
